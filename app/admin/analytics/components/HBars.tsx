...
export type HBar = { label: string; value: number | null; color?: string; valueLabel?: string; marker?: { value: number; label: string } };
...
export default function HBars({
  ...
  const markerValues = rows.map((r) => r.marker?.value ?? 0);
  const max = Math.max(marker?.value ?? 0, ...markerValues, ...values, 1) * 1.12;
  const fmt = (v: number | null, vl?: string) => (vl != null ? vl : v == null ? "—" : `${Number.isInteger(v) ? v : v.toFixed(1)}${unit}`);

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12, paddingTop: marker || rows.some((r) => r.marker) ? 14 : 2 }}>
      {rows.map((r, i) => {
        const rowMarker = r.marker ?? marker;
        const showLabel = r.marker ? true : i === 0;
        return (
          <div key={i} style={{ display: "grid", gridTemplateColumns: `${labelWidth}px 1fr 52px`, alignItems: "center", gap: 10 }}>
            ...
            <div style={{ position: "relative", height: 22, background: "var(--neutral-bg)", borderRadius: 6, overflow: "visible" }}>
              ...
              {rowMarker && showLabel ? (
                <div style={{ position: "absolute", left: `${Math.min(100, (rowMarker.value / max) * 100)}%`, top: -14, bottom: 0, borderLeft: "2px dashed var(--bad)", paddingLeft: 4 }}>
                  <span style={{ position: "absolute", top: -14, left: 4, fontSize: 10, color: "var(--bad)", whiteSpace: "nowrap" }}>{rowMarker.label}</span>
                </div>
              ) : rowMarker ? (
                <div style={{ position: "absolute", left: `${Math.min(100, (rowMarker.value / max) * 100)}%`, top: 0, bottom: 0, borderLeft: "2px dashed var(--bad)" }} />
              ) : null}
            </div>
